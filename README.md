...
- 인쇄용 PDF 에서 세로형 캡처(스마트폰·키오스크 화면)를 한 줄에 4개로 배치 — 빌드 시 `sips` 로 가로/세로 비를 재 0.8 미만이면 자동 적용. 2열일 때 세로 이미지는 높이에 맞춰 줄어 칸의 절반이 비어 있었으므로, 크기 손해 없이 줄 수만 반으로 준다(26→25쪽)
