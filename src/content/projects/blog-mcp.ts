...
export const ko: ProjectText = {
  ...
  techNotes: [
    ...
    },
    {
      ...
      body: 'MCP 서버는 LLM을 호출하지 않고 순수 함수로 도구만 노출합니다. 실제 자동화는 매일 1회 도는 트리거(GitHub Actions cron), 글을 실제로 쓰는 오케스트레이터(래퍼 스크립트 + 발행 관문 hook), 그리고 도구·리소스·프롬프트를 제공하는 MCP 서버로 분리했습니다. 모델 호출을 오케스트레이터 한 곳으로 몰아 서버를 무상태·무모델로 유지하는 경계 설계가 학습의 핵심이었습니다.',
    },
    {
      title: '무인 발행 파이프라인 (Phase 3)',
      body: '매일 KST 14:23 GitHub Actions cron이 오케스트레이터 래퍼(daily-post.sh)를 실행합니다. 래퍼는 모델의 자기 보고를 믿지 않고 파일시스템 변화로 성공을 판정하며, 실패 시 3회 재시도하고 5슬롯 섹션 로테이션(mcp→정처기→보안→MCP 동향→정보보안기사)으로 주제를 순환합니다. 같은 날 두 번 도는 것을 막는 중복 발행 가드도 워크플로에 두었습니다. 발행 직전에는 PreToolUse hook이 publish_post 호출을 결정론적으로 검문해(섹션 정합·slug 형식·태그 수) 위반 시 exit 2와 사유를 돌려줘 모델이 스스로 고치게 만듭니다. GITHUB_TOKEN 푸시는 다른 워크플로를 깨우지 못한다는 함정은 배포 워크플로를 workflow_call로 직접 호출해 풀었고, 매 실행의 관측치를 학습 기록 문서에 누적해 다음 개선의 입력으로 씁니다.',
    },
    {
      title: 'Mode R · 리서치 발행',
    ...
    },
    {
      ...
      body: '서버가 LLM을 부르지 않으므로 사실 검증 주체가 없어, 품질을 두 겹으로 지킵니다. 구조적 검증(강제)은 publish_post가 결정론적으로 막습니다 — 섹션 정합성, 본문 최소 분량(minChars), ## 소제목 존재, 그리고 follows/related는 물론 본문 안의 내부 링크까지 실존 여부와 형태(/blog/<slug>/)를 함께 검사해, 없는 글을 가리키거나 날짜 프리픽스가 빠진 주소가 있으면 발행을 거부합니다 — 끊긴 간선·링크 할루시네이션과 라이브 404를 발행 전에 막습니다. 서술 접지(유도)는 write_daily_post 프롬프트가 잡습니다 — 미검증 수치·API를 단정하지 않기, 발행 전 편집자 관점의 자기 검토 1회.',
    },
    {
      title: '성과 신호 되먹임 — 콘텐츠 루프 닫기',
      body: '발행 결과가 다음 주제 선택으로 되돌아오게 만들었습니다. suggest_topic이 Supabase 조회수를 읽어 영역(area)별 “조회수÷경과일” 평균으로 정렬에 가중치를 줍니다. 1차 키는 덜 다룬 영역을 먼저 쓰는 다각화를 그대로 두고 동률일 때만 성과순으로 갈라, 탐색을 죽이지 않게 했습니다. 표본이 적은 초기에 클릭 몇 번으로 순서가 뒤집히지 않도록 절대·밀도 두 겹의 cold-start 관문을 두었고, 키 미설정·네트워크 실패 시에는 조용히 기존 동작으로 떨어지도록(graceful degradation) 설계했습니다. 서버는 여기서도 LLM을 부르지 않고 결정론적 산술만 씁니다.',
    },
    {
      title: 'Astro 블로그 & 자동 배포',
      body: 'publish_post가 쓴 마크다운을 Astro Content Collection(프론트매터 스키마)으로 렌더합니다. 목차+스크롤스파이, Shiki 라이트/다크 이중 테마 코드블록, remark-cjk-friendly로 한글 강조 처리, 클라이언트 검색(⌘K), 태그·섹션 페이지, 관련 글·이전/다음, RSS·사이트맵·JSON-LD를 갖췄고, giscus 댓글과 익명 피드백 폼을 붙였습니다. 정적 사이트지만 조회수는 브라우저가 Supabase RPC를 직접 호출해 집계하고(세션 중복 방지), cron이 일일 스냅샷을 쌓아 히어로의 라이브 총 조회 스탯으로 보여줍니다. 레포명 변경으로 글 주소에 /blog/가 겹쳐 있던 것을 /blog/<slug>/로 정리하면서, 이미 색인된 옛 주소 109건은 콘텐츠 디렉터리를 읽어 리다이렉트 페이지를 자동 생성해 넘겼습니다(GitHub Pages는 서버 301을 줄 수 없어 meta refresh + canonical + noindex). CHANGELOG.md를 그대로 렌더하는 /changelog 페이지도 사이트에 두었습니다. GitHub Actions가 main 푸시 시 pnpm 워크스페이스를 빌드해 GitHub Pages로 배포합니다.',
    },
  ],
}
