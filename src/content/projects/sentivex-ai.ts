...
export const ko: ProjectText = {
  ...
  overview: [
    'SentiveX 플랫폼의 AI 축으로, 보안 인시던트를 실시간 자동 분석하는 상태 비저장 Fastify 서버입니다. 여러 EDR 벤더의 인시던트를 공통 타입(OCSF lite)으로 정규화하고, 역할별 보안 에이전트를 오케스트레이터로 실행해 위협을 다각도로 분석한 뒤 결과를 SSE로 스트리밍합니다. 저는 오케스트레이터·LiteLLM 게이트웨이·멀티테넌트 모델 관리·배포 전반을 맡았습니다(레포 최다 커밋 기여).',
